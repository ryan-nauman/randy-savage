...
module.exports = function (app, addon) {
  ...
  app.post('/spotify',
    addon.authenticate(),
    function (req, res) {
      var appUri = req.protocol + '://' + req.get('host');
      var spotifyUri = spotify.generateSpotifyLink(req.body.item.message.message);
      spotify.getMetadata(spotifyUri, appUri).then(function(data){
        hipchat.sendMessage(
          req.clientInfo,
          req.context.item.room.id,
          data.msg,
          {},
          data.card
        )
